...
abstract class BasesfBreadNav extends BaseObject  implements Persistent {
	...
	public function setRoute($v)
	{
		...
		if ($this->route !== $v) {
			$this->route = $v;
			$this->modifiedColumns[] = sfBreadNavPeer::ROUTE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->credential = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->catchall = ($row[$startcol + 5] !== null) ? (boolean) $row[$startcol + 5] : null;
			$this->tree_left = ($row[$startcol + 6] !== null) ? (int) $row[$startcol + 6] : null;
			$this->tree_right = ($row[$startcol + 7] !== null) ? (int) $row[$startcol + 7] : null;
			$this->tree_parent = ($row[$startcol + 8] !== null) ? (int) $row[$startcol + 8] : null;
			$this->scope = ($row[$startcol + 9] !== null) ? (int) $row[$startcol + 9] : null;
			$this->resetModified();
			...
			return $startcol + 10; // 10 = sfBreadNavPeer::NUM_COLUMNS - sfBreadNavPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[4] => $this->getCredential(),
			$keys[5] => $this->getCatchall(),
			$keys[6] => $this->getTreeLeft(),
			$keys[7] => $this->getTreeRight(),
			$keys[8] => $this->getTreeParent(),
			$keys[9] => $this->getScope(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[4], $arr)) $this->setCredential($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setCatchall($arr[$keys[5]]);
		if (array_key_exists($keys[6], $arr)) $this->setTreeLeft($arr[$keys[6]]);
		if (array_key_exists($keys[7], $arr)) $this->setTreeRight($arr[$keys[7]]);
		if (array_key_exists($keys[8], $arr)) $this->setTreeParent($arr[$keys[8]]);
		if (array_key_exists($keys[9], $arr)) $this->setScope($arr[$keys[9]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(sfBreadNavPeer::ROUTE)) $criteria->add(sfBreadNavPeer::ROUTE, $this->route);
